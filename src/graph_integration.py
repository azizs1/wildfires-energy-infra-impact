...
def analyze_overlay(infra_graph, wf_graph):
    ...
    wf_gdf = gpd.GeoDataFrame(wf_nodes, geometry="geometry", crs="EPSG:3310")

    fig, ax = plt.subplots(figsize=(12, 12))
    infra_gdf_display = infra_gdf.to_crs(epsg=3857)
    plot_infra(ax, infra_graph, infra_gdf_display)
    wf_gdf_display = wf_gdf.to_crs(epsg=3857)
    plot_wildfires(ax, wf_gdf_display)
